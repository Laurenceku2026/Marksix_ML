# ============================================================
# 六合彩AI智能选号工具 - 云端ML完整版 v7.0
# 第1部分：导入、配置、常量、7分区、规律加权函数
# 
# 新增功能：
#   1. 规律加权（边号、夹号、连号）
#   2. 可配置训练期数
...
# ==================== 页面配置 ====================
st.set_page_config(
    page_title="六合彩AI分析工具 - 云端ML完整版 v7.0",
    page_icon="🎰",
    layout="wide",
...
def parse_datetime_string(datetime_str: str) -> Optional[int]:
    ...
    for fmt in formats:
        try:
            ...
            delta = dt - base_date
            serial = delta.days + 2 + (delta.seconds / 86400)
...
# 版本：v7.0
# 包含：管理员登录、数据编辑器、Excel上传、清空数据、回测面板
# ============================================================
...
def parse_excel_file(uploaded_file) -> Optional[List[Dict]]:
    ...
    try:
        ...
        for col in df.columns:
            ...
            if '期' in col_str or 'period' in col_str:
                period_col = col
            elif '日期' in col_str or 'date' in col_str:
                date_col = col
            elif 'b1' in col_str or '正码1' in col_str:
                for i in range(1, 7):
                    target = f'b{i}' if 'b' in col_str else f'正码{i}'
                    for c in df.columns:
                        if str(c).strip().lower() == target:
                            number_cols.append(c)
                            break
            elif 'b7' in col_str or '特码' in col_str:
                special_col = col
        ...
        if special_col is None and len(df.columns) >= 9:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                special = None
                if special_col and pd.notna(row[special_col]):
                    special = int(row[special_col])
...
def run_backtest(draws: List[Dict], method_name: str, num_bets: int, num_count: int,
    ...
    """
    运行回测
    注意：具体算法实现在第3/4部分，这里只定义接口
    """
    if len(draws) < test_periods + max(method1_window, method2_window, method3_window, method4_window):
        st.error(f"数据不足：需要至少 {test_periods + max(method1_window, method2_window, method3_window, method4_window)} 期数据")
        return None
    ...
    for i in range(test_periods):
        ...
        # 根据方法名称调用对应的生成函数
        # 这些函数将在第3/4部分实现，这里先占位
        if "方法1" in method_name:
            # bets = generate_bets_method1_with_params(...)
            pass
...
def show_admin_page():
    ...
    # 构建 DataFrame
    if current_draws:
        display_draws = sorted(current_draws, key=lambda x: int(x.get('period', 0)) if str(x.get('period', 0)).isdigit() else 0, reverse=True)
        data_rows = []
    ...
    if st.button("▶️ 运行回测", type="primary", key="run_backtest"):
        ...
        # 这里调用回测函数，实际算法在后续部分实现
        st.info("回测功能将在第3/4部分完整实现，当前为占位界面")
        st.warning("请完成第3/4部分代码后，回测功能即可使用")


print("第2部分加载完成 (v7.0 - 管理员页面)")
print("=" * 60)
print("请确认第2部分代码，输入 CONFIRM 后继续第3部分")
print("=" * 60)
# ============================================================
# 第3部分：5种AI算法 + 规律加权 + 可配置训练期数
# 版本：v7.0
# 
# 包含：
#   方法1：当前方法（冷热码+和值预测+规律加权）
#   方法2：胆拖混合（胆码优先选加权号码）
#   方法3：LightGBM（规律特征 + 可配置训练期数）
#   方法4：XGBoost+NN（规律特征 + 可配置训练期数）
#   方法5：综合模式（投票 + 规律加权）
# ============================================================
...
def is_valid_combination(nums: List[int], target_sum: int, tolerance: int, last_draw_all: List[int]) -> bool:
...
def generate_one_combination(weights: Dict[int, float], num_count: int, target_sum: int, 
                              tolerance: int, last_draw_all: List[int]) -> Tuple[List[int], int]:
    """生成一组投注"""
    max_attempts = 10000
    for _ in range(max_attempts):
        selected = weighted_random_sample(weights, k=num_count)
        if is_valid_combination(selected, target_sum, tolerance, last_draw_all):
...
def generate_bets_method1_current(draws: List[Dict], num_bets: int, num_count: int,
    ...
    last_numbers = last_draw['numbers']
    last_special = last_draw.get('special')
    last_draw_all = last_numbers + ([last_special] if last_special else [])
    ...
    for i in range(num_bets):
        ...
        nums, total = generate_one_combination(weights, num_count, t, tolerance, last_draw_all)
        bets.append({
            'numbers': nums,
            'sum': total,
            'target': f'方法1(目标{t})',
...
def generate_bets_method3_lightgbm(draws: List[Dict], num_bets: int, num_count: int,
    ...
    """
    if not LGB_AVAILABLE:
        st.warning("⚠️ LightGBM未安装，降级使用方法2")
    ...
    if model is None:
        st.warning("LightGBM模型训练失败，降级使用方法2")
        return generate_bets_method2_hybrid(draws, num_bets, num_count, trend_window, random_seed, 50)
...
def build_advanced_features(draws: List[Dict], target_num: int) -> Optional[Dict]:
    ...
    # 分区特征
    features['zone'] = get_zone(target_num)
    features['is_hot_zone'] = 1 if get_zone(target_num) in get_hot_zones(calculate_zone_heat(draws)[0]) else 0
...
def generate_bets_method4_ensemble(draws: List[Dict], num_bets: int, num_count: int,
    ...
    """
    if not XGB_AVAILABLE or not SKLEARN_AVAILABLE:
        st.warning("⚠️ XGBoost或sklearn未安装，降级使用方法3")
    ...
    if ensemble is None:
        st.warning("XGBoost+NN集成训练失败，降级使用方法3")
        return generate_bets_method3_lightgbm(draws, num_bets, num_count, trend_window, random_seed, 100)
...
print("第3部分加载完成 (v7.0 - 5种AI算法 + 规律加权)")
print("=" * 60)
...
if st.session_state.get('draws_loaded') is None:
    with st.spinner("加载数据中..."):
        draws = load_recent_draws_from_supabase(limit=300)  # 只加载最近300期
        if draws:
            st.session_state['draws_loaded'] = draws
        else:
            st.session_state['draws_loaded'] = []
...
# ==================== 主页面标题 ====================
st.title("🎯 六合彩AI智能选号工具 - 云端ML完整版 v7.0")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("DFSS智能选号工具 v7.0")
    st.caption("更新: 2026-05-13")
    st.caption("新增: 规律加权 | 可配置训练期数 | 3种种子模式")


print("第4部分加载完成 (v7.0 - 主页面UI)")
print("=" * 60)
print("所有代码加载完成！六合彩AI智能选号工具 v7.0 已就绪。")
print("=" * 60)
